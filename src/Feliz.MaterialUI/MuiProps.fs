...
type avatar =
  ...
  static member inline alt(value: string) = Interop.mkAttr "alt" value
    /// Used to render icon or text elements inside the Avatar. `src` and `alt` props will not be used and no `img` will be rendered by default. This can be an element, or just a string.
  static member inline children(element: ReactElement) = prop.children element
    /// Used to render icon or text elements inside the Avatar. `src` and `alt` props will not be used and no `img` will be rendered by default. This can be an element, or just a string.
  static member inline children(elements: ReactElement seq) = prop.children elements
...
type clickAwayListener =
    /// The wrapped element. ⚠️ Needs to be able to hold a ref.
  static member inline children(element: ReactElement) = prop.children element
    /// The wrapped element. ⚠️ Needs to be able to hold a ref.
  static member inline children(elements: ReactElement seq) = prop.children elements
...
type select =
  ...
  static member inline autoWidth(value: bool) = Interop.mkAttr "autoWidth" value
    /// The option elements to populate the select with. Can be some `MenuItem` when `native` is false and `option` when `native` is true. ⚠️The `MenuItem` elements <strong>must</strong> be direct descendants when `native` is false.
  static member inline children(element: ReactElement) = prop.children element
    /// The option elements to populate the select with. Can be some `MenuItem` when `native` is false and `option` when `native` is true. ⚠️The `MenuItem` elements <strong>must</strong> be direct descendants when `native` is false.
  static member inline children(elements: ReactElement seq) = prop.children elements
